...
class PaintAdmin(admin.ModelAdmin):
    list_display = (
        "name", "category", "pricing_method", "base_type", "finish",
        "colour", "price_excl_vat", "price_incl_vat", "is_active", "created_at",
    )
    list_filter = ("category", "pricing_method", "base_type", "finish", "is_active")
    ...
    fieldsets = (
        ("Product Details", {
            "fields": ("name", "description", "category", "pricing_method", "base_type", "finish", "colour", "image"),
        }),
        ("Pricing", {
